...
def get_private_hands(history, cards):
    ...
    if num_rounds == NUM_BETTING:
        hand_one += cards[4+(NUM_BETTING-1)*2]
        hand_two += cards[5+(NUM_BETTING-1)*2]

    return [hand_one, hand_two]


def get_draw_cards(history, cards):
    # print(history)
    hands = re.findall('\(([^)]+)', history)

    return cards[4+len(hands)*2:6+len(hands)*2]
